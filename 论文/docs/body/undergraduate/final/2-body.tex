...
我们取网格数$N=3200$、时间步长$t=1.2e-3$、收敛条件$\epsilon = 0.001$和计算空间$V_h^2$的数值解作为真实解，我们有\autoref{tab:IMEXLDGerror:1}、\autoref{tab:IMEXLDGerror:2}和\autoref{tab:IMEXLDGerror:3}的误差估计，其中，我们有几点要说明的地方：表中所有案例均选择时间步长$\Delta t = 1.2e-3$和收敛条件$\epsilon=0.001$，模的取法与误差估计中相同；从\autoref{tab:IMEXLDGerror}可以看出时间项的影响远小于空间项，我们在计算精度时没有排除时间项；由于算力限制，我们没有取更小的时间步长，这将导致时间项的影响随着网格加密而逐渐增加，导致无法精确计算出算法的精度；计算模时为了简便乘以了某个常数，但这不影响计算精度。
\begin{table}
    \begin{tabularx}{\textwidth}{@{} *5{X} @{}}
        ...
        \midrule
        25  & 4.42e-03                           & ----------                         & 4.42e-03                  & ---------- \\
        50  & 1.21e-03                           & 1.86811279                         & 1.21e-03                  & 1.86811272 \\
        100 & 1.65e-04                           & 2.87652077                         & 1.65e-04                  & 2.87652080 \\
        200 & 2.16e-05                           & 2.93417415                         & 2.16e-05                  & 2.93417539 \\
        400 & 2.79e-06                           & 2.95007100                         & 2.79e-06                  & 2.95009136 \\
        800 & 3.41e-07                           & 3.03452119                         & 3.41e-07                  & 3.03462336 \\
        \bottomrule
    \end{tabularx}
...
分析\autoref{tab:IMEXLDGerror:1}、\autoref{tab:IMEXLDGerror:2}和\autoref{tab:IMEXLDGerror:3}中数据我们可以得到：对于一阶IMEX LDG格式，对于$V^1$我们获得了符合期望的一阶精度，对于更高次计算空间，我们的误差估计中的空间项$Ch^{k+1}$以指数衰减至$Ch^{k+1} \ll C\Delta t$，我们仅得到$O(1)$的精度，尽管数据看似很差，但\autoref{tab:IMEXLDGerror:1}的结果是符合我们预期的；对于二阶和三阶IMEX LDG格式，在计算空间$V^0, V^1, V^2$中我们分别得到了一阶、二阶、三阶精度，而对于从网格数$N=25$到网格数$N=50$这两个网格上求得误差明显差于更细的网格，可能的原因是因为当网格过粗时，投影得到的初值函数误差过大。此外注意到，对于随着网格加密，我们的时间步长保持不变，但依然可以得到收敛的结果，这证明了IMEX方法的无条件稳定性。在我们的验证中，$t=1.2e-3$是一个接近临界的时间步长，当选取更大的时间步长如$t=3.0e-3$时，我们的数值解随着迭代的进行很快就发散了，更精确的时间步长可以通过二分法逐渐求出，这不是本文讨论的重点，同时为了节省算力，此处不做过多展开。
...
\begin{table}
    \begin{tabularx}{\textwidth}{@{} *7{X} @{}}
        ...
        \midrule
        25  & 3.04e+07                  & \text{——}                 & 8.93e+06                  & \text{——} & 4.02e+06              & \text{——} \\
        50  & 1.96e+07                  & 0.6319                    & 4.14e+06                  & 1.1086    & 7.20e+05              & 2.4808    \\
        100 & 1.12e+07                  & 0.8066                    & 1.19e+06                  & 1.8012    & 3.07e+05              & 1.2318    \\
        200 & 5.87e+06                  & 0.9345                    & 4.05e+05                  & 1.5521    & 2.92e+05              & 0.0728    \\
        400 & 2.98e+06                  & 0.9792                    & 3.02e+05                  & 0.4245    & 2.91e+05              & 0.0039    \\
        800 & 1.52e+06                  & 0.9690                    & 2.92e+05                  & 0.0463    & 2.91e+05              & 0.0004    \\
        \bottomrule
    \end{tabularx}
    \caption{一阶IMEX LDG格式误差估计。网格数N，收敛阶Order，计算空间$V^k, k=0,1,2$，绝对误差$||n_h-n||+||q_h-q||$。}
...
\begin{table}
    \begin{tabularx}{\textwidth}{@{} *7{X} @{}}
        ...
        \midrule
        25  & 9.94e-02                  & \text{——}                 & 3.23e-02                  & \text{——} & 4.42e-03                  & \text{——} \\
        50  & 5.51e-02                  & 0.8548                    & 9.31e-03                  & 1.7952    & 1.21e-03                  & 1.8681    \\
        100 & 2.80e-02                  & 0.9752                    & 2.45e-03                  & 1.9244    & 1.65e-04                  & 2.8765    \\
        200 & 1.41e-02                  & 0.9909                    & 6.22e-04                  & 1.9796    & 2.16e-05                  & 2.9342    \\
        400 & 7.07e-03                  & 0.9965                    & 1.58e-04                  & 1.9780    & 2.79e-06                  & 2.9501    \\
        800 & 3.54e-03                  & 0.9994                    & 3.90e-05                  & 2.0174    & 3.41e-07                  & 3.0345    \\
        \bottomrule
    \end{tabularx}
...
\begin{table}
    \begin{tabularx}{\textwidth}{@{} *7{X} @{}}
        ...
        N   & $\frac{||n_h-n||}{||n||}$ & Order                     & $\frac{||n_h-n||}{||n||}$ & Order     & $\frac{||n_h-n||}{||n||}$ & Order     \\
        \midrule
        25  & 9.96e-02                  & \text{——}                 & 3.23e-02                  & \text{——} & 4.42e-03                  & \text{——} \\
        50  & 5.51e-02                  & 0.8548                    & 9.31e-03                  & 1.7952    & 1.21e-03                  & 1.8681    \\
        100 & 2.80e-02                  & 0.9752                    & 2.45e-03                  & 1.9244    & 1.65e-04                  & 2.8765    \\
        200 & 1.41e-02                  & 0.9909                    & 6.22e-04                  & 1.9796    & 2.16e-05                  & 2.9342    \\
        400 & 7.07e-03                  & 0.9965                    & 1.58e-04                  & 1.9780    & 2.79e-06                  & 2.9501    \\
        800 & 3.54e-03                  & 0.9994                    & 3.90e-05                  & 2.0174    & 3.41e-07                  & 3.0346    \\
        \bottomrule
    \end{tabularx}
