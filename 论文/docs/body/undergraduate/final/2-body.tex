...
第二种基函数选择$I_j = (x_{j-1/2}, x_{j+1/2})$上正交Legendre多项式基
\begin{align*}
...
\autoref{fig:TVDRKLimiter}是添加了改进后的minmod限制器和没有添加minmod限制器的图像，由于我们的解具有良好光滑性，同时计算空间选择为$V^2$，因此是否添加限制器基本上不会产生影响。
\begin{figure}
	\centering
	\begin{minipage}{0.45\linewidth}
		\centering
