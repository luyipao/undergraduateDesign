...
%%minmode limiter
h = mesh(2) - mesh(1);
M = 2 / 3 * 249*20*pi*20*pi * 20 * pi * 10e2 * h^2;
auxqCoeffs = zeros(N*(n+1),1);
for j = 1:N
    func1 = func1Cells{j};
    ur = func1(mesh(j+1)) - localAverageAuxq(j);
    ul = localAverageAuxq(j) - func1(mesh(j));
    urmod = localAverageAuxq(j) + minmod([ul localAverageAuxq(mod(j,N)+1)-localAverageAuxq(j) localAverageAuxq(j)-localAverageAuxq(mod(j-2+N,N)+1)], M);
    ulmod =  localAverageAuxq(j) - minmod([ur localAverageAuxq(mod(j,N)+1)-localAverageAuxq(j) localAverageAuxq(j)-localAverageAuxq(mod(j-2+N,N)+1)], M);
